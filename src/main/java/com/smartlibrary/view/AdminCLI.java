...
public class AdminCLI {
    ...
    public AdminCLI(Scanner scanner) {
        this(scanner, new Admin(0, "SYSTEM", "System Admin", "admin@smartlibrary.com"));
    }
    public AdminCLI(Scanner scanner, Admin admin) {
    ...
    public void start() {
        ...
        while (running) {
            ...
            switch (scelta) {
                case 1:
                    handleVisualizzaCatalogo();
                    break;
                case 2:
                    handleGestioneRisorsa();
                    break;
                case 3:
                    handleRimozione();
                    break;
                case 4:
                    handleAssociaLibroCorso();
                    break;
                case 5:
                    handleRimuoviAssociazione();
                    break;
                case 6:
                    handleVisualizzaCorsi();
                    break;
                case 0:
                    running = false;
    ...
    private void handleAggiornamentoEsistente(String isbn, ElementoBibliotecario esistente) {
        ...
        if (esistente instanceof Libro) {
            ...
            if (aggiorna) {
                ...
                String titoloAggiornato = currentAdmin.aggiornaQuantita(itemDAO, isbn, copieAdd);
        ...
        if (nuovaAssociazione) {
            ...
            if (courseDAO.existsAssociazione(isbn, corso, anno)) {
                ...
                currentAdmin.associaCorso(courseDAO, isbn, corso, anno);
    ...
    private void handleInserimentoNuovo(String isbn) {
        ...
        try {
            currentAdmin.aggiungiRisorsa(itemDAO, isbn, tipo, titolo, autore, corso, anno, param);
    ...
    private void handleRimozione() {
        ...
        if (conferma) {
            boolean successo = currentAdmin.eliminaRisorsa(itemDAO, isbn);
            if (successo) {
                System.out.println(">> Libro rimosso con successo");
                aggiornaCorsiMap();
            } else {
                System.out.println(
                        ">> Impossibile rimuovere il libro \n controlla se ci sono prestiti attivi associati a questo ISBN");
            }
        } else {
    ...
    private void handleAssociaLibroCorso() {
        ...
        if (courseDAO.existsAssociazione(isbn, corso, anno)) {
            ...
            boolean inserito = currentAdmin.associaCorso(courseDAO, isbn, corso, anno);
            if (inserito) {
                System.out.println(
                        ">> Associazione creata: " + item.getTitolo() + " → " + corso + " (Anno " + anno + ")");
                aggiornaCorsiMap();
            } else {
    ...
    private void handleRimuoviAssociazione() {
        ...
        if (conferma) {
            boolean rimosso = currentAdmin.rimuoviAssociazione(courseDAO, isbn, corso, anno);
            if (rimosso) {
